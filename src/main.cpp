   ...
   class application
   {
    ...
    public:
      ...
      void init()
      {
         ...
         fruit.position = {0, 0}; // view.center(); // world position
                                  // camera = fruit.position;
      }
    ...
    private:
      ...
      {
         const auto delta = fruit.position + decltype(fruit.position){dx, dy};
         spdlog::debug("movement delta ({},{}) ", delta.y, delta.x);

         const auto move_allowed = [this](point pos) {
            return true;
            return std::visit([p = pos](auto map) { return std::get_if<blocked>(&map.entities[p.y][p.x]) == nullptr; },
                              maps);
         };

         const auto map_bounding_rect = rect{{0, 0}, std::visit([](auto map) { return extent(map); }, maps)};

         const auto check_map_border = [](point pos, extents ex) {
            return pos.x <= ex.width - 1 && pos.y <= ex.height - 1;
         };

         if (map_bounding_rect.is_inside(delta) && move_allowed(delta) &&
             check_map_border(delta, map_bounding_rect.extent)) {
            fruit.position = delta;
         } else {
         ...
         if (!gameOver) {
            ...
            if (!pause) {
               ...
               const auto is_under_portal = [this](point pos) {
                  return std::visit(
                     [p = pos](auto map) { return std::get_if<portal>(&map.entities[p.y][p.x]) != nullptr; }, maps);
               };
                  ...
                  return std::visit(
                     [this, p = pos](auto map) {
                        if (const auto value = std::get_if<portal>(&map.entities[p.y][p.x]); value) {
                           ...
                           // select new map
                           std::visit([this](auto e) { maps = tile_map_by_tag_t<decltype(e), decltype(maps)>{}; },
                                      value->target);

                           //! \todo  position map

                           const auto map_size = rect{{0, 0}, std::visit([](auto m) { return extent(m); }, maps)};
                           ...
                           if (view.is_inside(map_size)) {
                              ...
                              // fruit.position = map_offset + value->position;
                           } else if (view.height() < map_size.height() && view.width() > map_size.width()) {
                              spdlog::debug("new map is higher than view, therefore center around player");
                              map_offset = view.center() - value->position + point{0, 1};
                              // fruit.position = view.center(); // map_offset + value->position - point{0,1};
                           } else {
                              spdlog::debug("new map is GREATER than view,!!!");
                              map_offset =
                                 view.center() - value->position + point{1, 0}; // point{6,-9}; // - value->position;
                                                                                // fruit.position = view.center();
                           }
               ...
               if (IsKeyPressed(KEY_ENTER) || IsKeyPressed(KEY_KP_ENTER) ||
                  ...
                  trigger_portal(screen_to_map_pos(fruit.position));
               }
      ...
      void draw() const noexcept
      {
         ...
         if (!gameOver) {
            ...
            // Draw to player
            DrawRectangleV(world_to_screen(fruit.position - camera) + (screen_offset / 2), fruit.size, fruit.color);

            // Draw to camera
            // DrawRectangleV(world_to_screen(camera) + (screen_offset / 2), fruit.size, PINK);
      ...
      void draw_game_debug_information() const
      {
         ...
         DrawText(fmt::format("Player pos: ({},{})", fruit.position.x, fruit.position.y).c_str(), left,
      ...
      void draw_grid() const noexcept
      {
         ...
         for (auto i = 0; i < cfg.window.width() / SQUARE_SIZE + 1; ++i) {
            DrawLineV({SQUARE_SIZE * i + ho.x, ho.y}, {SQUARE_SIZE * i + ho.x, cfg.window.height() - ho.y}, LIGHTGRAY);
         }
         for (auto i = 0; i < cfg.window.height() / SQUARE_SIZE + 1; i++) {
            DrawLineV({ho.x, SQUARE_SIZE * i + ho.y}, {cfg.window.width() - ho.x, SQUARE_SIZE * i + ho.y}, LIGHTGRAY);
         }
      }
...
int main([[maybe_unused]] int argc, [[maybe_unused]] char* argv[])
{
   try {
      ...
      auto app = orpg::application{{.window_title = "foobar"}};
      app.init();
